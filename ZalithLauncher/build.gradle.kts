...
android {
    ...
    signingConfigs {
        ...
        // Fix: Make Release use Debug's key to skip password errors
        create("releaseBuild") {
            initWith(debugConfig)
        }
    }
    ...
    buildTypes {
        release {
            isMinifyEnabled = false
            isShrinkResources = false
            // This now points to our password-free releaseBuild
            signingConfig = signingConfigs.getByName("releaseBuild")
            proguardFiles(
                getDefaultProguardFile("proguard-android-optimize.txt"),
                "proguard-rules.pro"
    ...
    // Fix: Force only Arm64 builds to save time and prevent errors
    splits {
        abi {
            isEnable = true
            reset()
